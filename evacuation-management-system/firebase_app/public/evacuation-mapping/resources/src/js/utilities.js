...
function optimizeMap(road_map) {
    ...
    coordinates.forEach((value_1, index_1) => {
        ...
        class DistanceCalculator {
            ...
            static R = 6371e3;
            static degToRad = function (deg) { return deg * (Math.PI / 180); };
            static calculateDistance = function (coordinates1, coordinates2) {
                if (coordinates1.lat === coordinates2.lat && coordinates1.long === coordinates2.long) {
                    return 0;
                }
                var a = Math.pow(Math.sin(this.degToRad(coordinates2.lat - coordinates1.lat) / 2), 2)
                + Math.cos(this.degToRad(coordinates1.lat)) * Math.cos(this.degToRad(coordinates2.lat))
                * Math.pow(Math.sin(this.degToRad(coordinates2.long - coordinates1.long) / 2), 2);
                var c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
                return this.R * c;
            };
            static instanceOfCoordinates = function (object) { return 'lat' in object && 'long' in object; };

            static calculate = function (coordinates1, coordinates2, unit) {
                if (unit === void 0) { unit = SUPPORTED_UNIT.m; }
                if (typeof coordinates1 !== 'object' || !this.instanceOfCoordinates(coordinates1)) {
                    throw new Error('[distance-calculator-js]: coordinates1 is not an instance of Coordinates');
                }
                if (typeof coordinates2 !== 'object' || !this.instanceOfCoordinates(coordinates2)) {
                    throw new Error('[distance-calculator-js]: coordinates2 is not an instance of Coordinates');
                }
                var m = this.calculateDistance(coordinates1, coordinates2);
                switch (unit) {
                    case this.SUPPORTED_UNIT.km:
                    return (m * 0.001).toFixed(3); //Math.round(m * 0.001);
                    case this.SUPPORTED_UNIT.M:
                    return (m * 0.000621371192).toFixed(3);//Math.round(m * 0.000621371192);
                    case this.SUPPORTED_UNIT.ft:
